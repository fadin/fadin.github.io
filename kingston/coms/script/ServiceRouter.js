function ServiceRouter()
{
	...
	this.$_REQUEST = function(key)
	{
		...
		if(ret) ret = ret.replace(/%22/g, "\"").replace(/%27/g, "'").replace(/%20/g, " ");
		return ret;
	}

	this.init = function(srURL, systemName, bPost, bDebug, bShowErrors, preProcessHTML)
	{
		if(!bDebug && document.URL.indexOf("debug=true")>0) bDebug = true;
		
		this.bLocal = !this.Store && !(document.URL.indexOf("local=false")>=0);
		
		this.bCache = document.URL.indexOf("cache=true")>0; /*|| this.bLocal;*/ /*are we always caching?*/
		...
		if(!srURL)
		{
			...
			if(!this.srURL)
			{
				if(document.URL.indexOf(":10080/nammour.com/")>0 || document.URL.indexOf("192.168.33.1/nammour.com/")>0)
				{
		...
		for (var i = 0; i < ar.length; i++) {
			...
			for (var j = 0; j < fields.length; j++) {
			    ...
			    for(var f=0; f<fieldParts.length; f++){
			        if(fieldParts[f]) value = value[fieldParts[f]];
			    }
				if (this.Equals(fields[j].key, value)) {
					// found
					fIndex = j;
	...
	this.compile = function(arModules){
		var calls = [];
		$.each(arModules, (i, m) => calls.push(sr._("ContentManager.cmsHTMLPageFindall", null, {Page: m})));
...
*/
	...
	this.ShowMessage = function(s)
	{
        if(typeof noty !== 'undefined'){
            noty({text: s, type: 'success', timeout: 2000});
        }else{
            // the default
			try{
...
//		if(this.bDebug) alert(s);
	...
	{
		var ret = "/*" + this.CRNL + "<![CDATA[" + this.CRNL + "PARAMETERS" + this.CRNL + "]]>" + this.CRNL + "*/" + this.CRNL + this.CRNL + "<p>"+this.CRNL;
	...
	this.PostCache = function(index, fCallBack)
	{
		...
		if(window.JQuery){
			...
			$.each(this.arCache, (i, c)=>{
				calls.push(this._("ContentManager.cmsDumpCache", null, {Date: c.Date, Code: c.hash, Result: c.Response}, "/nammour.com/store/" + (window.company?window.company.Code:""), true));
			});
			...
			this._("ContentManager.cmsDumpCache", function(r)
			{
				//window.sr.ShowObject(r, true);
				window.sr.PostCache(index-1, fCallBack);
			}, {Date: c.Date, Code: c.hash, Result: c.Response}, "/nammour.com/store/" + (window.company?window.company.Code:""), true);
		}
	}
	...
	this.processResponse = function(readyState, callBack, responseText, url)
	{
		...
		var sMethodName = url.substring(url.indexOf("name=")+5, url.indexOf("&", url.indexOf("name=")+5));

		try
		{
			if(this.bCache && sMethodName!="ContentManager.cmsMethodResultInsert")
			{
			...
			if(server_time) this.timeDifference = new Date().getTime() - server_time.getTime();
			...
			if(method_name && method_name=="ContentManager.cmsMethodResultFind" && ret.Completed>ret.Date){
				for(var i=0; i<this.arCache.length; i++){
					if(this.arCache[i].URL.indexOf(ret.StoredMethod.Name)<0) continue;
					if(this.arCache[i].Response.indexOf(ret.Code)<0) continue;
		 	...
		 	if(callBack!=null)
		 	{
				var callRet = null;
		 		...
		 		{
		 			this.ShowError("Error in Callback:\n\n" + callBack + "\n\n" + e.message);
		 		}
	 			if((typeof method_name !== 'undefined') && (typeof sMethodName !== 'undefined') && sMethodName!=method_name)
	 			{
	 				this.ShowError("Mismatch in Method between server and client:\nServer Method Name: " + method_name + "\nClient Method Name: " + sMethodName);
	 			}
		 		(this.fLoadingEnd || (function(){window.sr.resetCursor();}))();
				return callRet || ret;
		 	}
			else
			{
	...
	{
		return o!=null && typeof(o)=="object" && !(o.constructor.toString().indexOf("function Date()") > -1) && !(o.constructor.toString().indexOf("Array()") > -1);
	}
	...
	this._toJS = function(o)
	{
			...
			{
					case "string":
							s += "\""+this.myReplace(this.escapeString(o), ["\": ,"], ["\": null,"])+"\"";break;
					case "number":
							s += o;break;
					case "boolean":
							s += (o?"true":"false"); break;
					case "object":
							// Date
							if(o.constructor.toString().indexOf("function Date()") > -1)
							{
									...
									var month = (o.getMonth()).toString(); month = (month.length == 1) ? "0" + month : month;
									...
									s += "new Date("+year+","+month+","+date+","+hours+","+minutes+","+seconds+","+milliseconds+")";
							}
							// Array
							...
							else
							{
								s += "{";
						 		...
						 		if(s.lastIndexOf(", ")==s.length-2) s = s.substring(0, s.length-2);
						 		s += "}";
							}
							break;
					case "function":
	...
	this._toXML = function(o, level)
	{
		...
		{
			case "string":
				s += "<![CDATA[" + o/*.replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;")*/ + "]]>"; break;
			case "number":
			case "boolean":
				s += o.toString(); break;
			case "object":
				// Date
				if(o.constructor && o.constructor.toString().indexOf("function Date()") > -1)
				{
					var year = o.getFullYear().toString();
					var month = (o.getMonth() + 1).toString(); month = (month.length == 1) ? "0" + month : month;
					var date = o.getDate().toString(); date = (date.length == 1) ? "0" + date : date;
					...
					s += month + "/" + date + "/" + year + " " + hours + ":" + minutes + ":" + seconds;
				}
				// Array
				else if(o.constructor && o.constructor.toString().indexOf("Array()") > -1)
				{
					for(var p in o)
					{
						...
						if(!isNaN(p))	 // linear array
						{
							...
							//s += "<" + type + ">" + this._toXML(o[p]) + "</" + type + ">";
							s += this._toXML(o[p], level++);
						}
						else
						{
							// associative array
							s += "<" + p + this.coop(o, p) + ">" + this._toXML(o[p], level++) + "</" + p + ">";
						}
						s += "</Object>"+this.CRNL;
					}
				}
				...
				else
				{
					...
					else if(false && o.Id) // this is creating a problem, better send all the object
					{
					...
					else
					{
				 		...
				 		{
				 			if(p=="OPERATORS" || p=="ORS") continue;
				 			s += "<" + p + this.coop(o, p) + this.OR(o,p) + ">" + this._toXML(o[p], level++) + "</" + p + ">";
				 		}
					}
				}
				break;
	...
	{
		return ((o.ORS&&o.ORS[p])?" OR='"+this.myReplace(o.ORS[p],['<', '>'],['&lt;', '&gt;'])+"'":"");
	}
	
	this.coop = function(o, p)
	{
		return ((o.OPERATORS&&o.OPERATORS[p])?" coop='"+this.myReplace(o.OPERATORS[p],['<', '>'],['&lt;', '&gt;'])+"'":"");
	}
	...
	this._ = function(fun, callBack)
	{
		...
		{
			alert("The following errors were found in your data:\n" + this.s_ErrorMessages);
			this.s_ErrorMessages = "";
			return null;
		}
		
		//var postData = "System.Collections.ArrayList ret = new System.Collections.ArrayList();"+this.CRNL;
		(this.fLoadingStart || (function(){document.body.style.cursor = 'wait';}))();
		...
		if(false && fun=="CorporateMeasures.comGroupCounts"){
			console.log(hCode);
		...
		if(!this.bLocal)
		{
			...
			try
			{
				var fResponseText = function(responseText, fCallBack)
				{
					...
					window.sr.ShowDebug("Response From Server:\n" + responseText);
		
					var data = window.sr.runScript(responseText);

					if(server_time) window.sr.timeDifference = new Date().getTime() - server_time.getTime();
					...
					if(_exception)
					{
				 	...
				 	if(fCallBack!=null)
				 	{
				 		try
				 		{
				 			fCallBack(ret || data, _exception);
						}
						catch(e)
						{
				 			window.sr.ShowError("Error in Callback:\n\n" + fCallBack + "\n\n" + e.message);
						}
					}
			 		(window.sr.fLoadingEnd || (function(){window.sr.resetCursor();}))();
					return ret || data;
				};
				...
				for(var i=0; i<this.arCache.length; i++)
				{
					...
					{
						return fResponseText(this.arCache[i].Response, callBack);
					}
				}
				var _theUrl = (this.Store||("/store/" + ((window.company&&window.company.Code)?(window.company.Code+"/"):"/")))+hCode+".js?rand=" + Math.random();
				if(!callBack){
					var data = null;
					this.Get(_theUrl).always(function(responseText){
						...
						if(window.sr.ActiveRequest && data)
						{
							...
							window.sr.arCache[window.sr.arCache.length] = window.sr.ActiveRequest;
						}
					});
		...
		{
			return this.sendXML(this.srURL + "&name=" + fun, postData, callBack);
		}
	}
	...
	this._url = function(fun, filename)
	{
		...
		return this.srURL + "&name=" + fun + "&rand=" + Math.random()
			//+ _args
			+ "&POSTCODE=" + this.toHex(postData)
	...
	this._json = function(fun)
	{
		...
		return this.srURL + "&name=" + fun + "&rand=" + Math.random()
			+ "&json=clean&POSTCODE=" + this.toHex(this.param(args));
	}
	...
	this.runCode = function(csCode, jsCode, pXML, callBack)
	{
		...
		postData += "m.BodyCode = \"" + this.myReplace(csCode, ["\"", "\r", "\n"], ["\\\"", "\\r", "\\n"]) + "\";"+this.CRNL;
		postData += "m.ReturnCode = \"" + this.myReplace(jsCode, ["\"", "\r", "\n"], ["\\\"", "\\r", "\\n"]) + "\";"+this.CRNL;
	...
	this.Post = function(url, callBack, postData, headers, username, password, contentType)
	{
		...
		if(xmlHTTP)
		{
			...
			if(crossDomain) xmlHTTP.setRequestHeader('Access-Control-Allow-Origin', location.origin);
			if(contentType) xmlHTTP.setRequestHeader("Content-Type", contentType);
			if(username) xmlHTTP.setRequestHeader("Authorization", "Basic " + btoa("username:password"));

			if(headers) for(var i=0; i<headers.length; i++) xmlHTTP.setRequestHeader(headers[i].key, headers[i].value);
	...
	this.Deferred = function(){
		window.resolve = window.resolve || function(r){
			window.deferred?window.deferred.resolve(r):console.log("window.deferred is null");
		};
		...
		if(xmlHTTP)
		{
				...
				window.xmlHTTP.onreadystatechange = function()
					{
						...
						{
							window.SR.processResult(window.SR.processResponse(window.xmlHTTP.readyState, callBack, window.xmlHTTP.responseText, url), url);
						}
					};
			}
			...
			if (true || typeof window.jQuery === 'undefined') {
				...
				if(this.bPost || postData.length>nMaxURLLength)
				{
					...
					window.xmlHTTP.setRequestHeader("Content-Type", 'multipart/form-data; boundary=--------------'); // fool cross-domain checking in chrome
					window.xmlHTTP.send("POSTDATA=\r\n" /*+ "<?xml version=\"1.0\" encoding=\"utf-8\" standalone=\"yes\"?>\r\n"*/ + postData);
				}
				else
				{
					xmlHTTP.open("GET", url + "&POSTCODE=" + this.toHex(postData), callBack!=null);
					xmlHTTP.send(null);
				}
				...
				if(callBack==null)
				{
					if (xmlHTTP.readyState==4)
					{
						...
						this.processResult(this.processResponse(xmlHTTP.readyState, callBack, xmlHTTP.responseText, _url), _url);
					}
				}
			} else {
			  // jQuery is available
			  ...
			  alert("FOR AJAX: " + a_url);
				...
				$.ajax({
					...
					this.processResult(this.processResponse(4, callBack, data, a_url), a_url);
				});
			}
			return (!callBack && window.deferred)?window.deferred.promise():true;
		}
		else return (!callBack && window.deferred)?window.deferred.promise():false;
	}
	...
	{
		return new Date(o.getFullYear(), o.getMonth()+m, o.getDate(), o.getHours(), o.getMinutes(), o.getSeconds());
	}
	
	this.isDate = function(o)
	{
		return o!=null && o.constructor!=null && (o.constructor.toString().indexOf("function Date(") > -1);
	}
	...
	{
		if(!this.isDate(o)) return ""; // avoid nulls
		var hours = "" + o.getHours()%12;
		var minutes = "" + o.getMinutes();
		if(hours.length<2) hours = "0" + hours;
		if(minutes.length<2) minutes = "0" + minutes;
		return hours + ":" + minutes + " " + (o.getHours()>=12?"PM":"AM");
	}
	...
	this.setProperty = function(cid, s, prop, def)
	{
		...
		try
		{
			this.runScript("$('"+o.id+"')." + prop + " = window.propertyValue;");
		}
		catch(e)
		{
	...
	this.validate = function(o, type, config)
	{
		...
		if((tag=="span" || tag=="a") && (o.getAttribute("from") || o.getAttribute("lookup")))
			{
		...
		bValid = bValid && this.runScript("window.cms.sr.valid"+type+"(window.validationValue)");
	...
	this.Equals = function(o1, o2)
	{
		...
		if(o1 && o1.__OBJECTID && o2 && o2.__OBJECTID && o1.__OBJECTID==o2.__OBJECTID) return true;
		if(o1 && o1.__ROWID && o2 && o2.__ROWID && o1.__ROWID==o2.__ROWID) return true;
